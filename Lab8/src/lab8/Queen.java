...
public class Queen extends Piece {
  ...
  public Queen(String name, PieceColor pieceColor, int currentRow, int currentCol) {
  ...
  public void move(Board board, int tarRow, int tarCol) {
    ...
    if (!(tarRow == curRow || tarCol == curCol || Math.abs(tarRow - curRow) == Math.abs(tarCol - curCol))) {
        System.out.println("Queens can move horizontally, vertically, or diagonally on the board");
        return;
    }
    ...
    int steps = Math.max(Math.abs(tarRow - curRow), Math.abs(tarCol - curCol));
